...
int main(int argc, const char **argv) {
  ...
  cv::CommandLineParser parser(
      ...
      "{use_dnn    n | 1 | Flag to use DNN models, set 1/true for true "
      "value or 0/false for false}"
      "{display    d | 1 | Flag to display the output frames with "
      "bounding boxes (1/true or 0/false)}"
      "{verbosity  v | 1 | verbosity level (0 - 2): higher value prints "
  ...
  main_processor.SetParams(model_path, camera_device, verbosity_level, use_dnn,
                           display);
  main_processor.Run();
}