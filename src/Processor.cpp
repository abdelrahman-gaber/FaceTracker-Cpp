...
void Processor::SetParams(cv::String model_pth, int camera_id, int verbosity,
                          bool use_dnn, bool display) {
...
void Processor::Run() {
  ...
  _futures.emplace_back(
      std::async(std::launch::async, &Processor::Capture, this));
  // Start the display thread
  _futures.emplace_back(
      std::async(std::launch::async, &Processor::Display, this));
  ...
  while (true) {
    ...
    // std::cout << "display_msg_queue size:" <<
    // _display_msg_queue.GetQueueSize() << "\n";
  }
}
...
void Processor::Capture() {
  ...
  std::string fourcc_str =
      cv::format("%c%c%c%c", fourcc & 255, (fourcc >> 8) & 255,
                 (fourcc >> 16) & 255, (fourcc >> 24) & 255);
  std::cout << "Input video format: " << fourcc_str << "\n";

  // set max frame size to 2 so you get the latest frames from the camera
  // whatever the speed of the Processor
...
void Processor::Display() {
  ...
  // cv::VideoWriter video("capture_record.mp4",
  // cv::VideoWriter::fourcc('a','v','c','1'), 24, cv::Size(640, 480));
  if (_display) {
  ...
  while (true) {
    ...
    avg_times.Update(message.capture_time, message.preprocess_time,
                     message.detection_time, display_time);

    if (_verbosity >= 2) {
      std::cout << "Frame " << message.frame_number
                << " : Capture: " << message.capture_time * 1000
                << " ms .. Preprocessing: " << message.preprocess_time * 1000
                << " ms .. Detection: " << message.detection_time * 1000
                << " ms .. Display: " << display_time * 1000
                << " ms ... Avg display time: "
                << avg_times.avg_overall_time * 1000 << " ms \n";
    }

    // each 100 frames
    if (_verbosity >= 1 and message.frame_number % 100 == 0) {
      std::cout << "Frame " << message.frame_number
                << " : Avg capture time: " << avg_times.avg_capture_time * 1000
                << " ms .. Avg preprocess time: "
                << avg_times.avg_preprocess_time * 1000
                << " ms .. Avg detection time: "
                << avg_times.avg_detection_time * 1000
                << " ms .. Avg overall time: "
                << avg_times.avg_overall_time * 1000 << " ms --> "
                << 1.0 / avg_times.avg_overall_time << " fps \n";
    }
